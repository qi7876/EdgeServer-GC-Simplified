...
DataSender::DataSender(SSLConnection* dataSecureChannel)
{
    ...
    sendChunkBuf_.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendChunkBatchSize_ * sizeof(Chunk_t));
    ...
    sendEncBuffer_.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendChunkBatchSize_ * sizeof(Chunk_t));
...
void DataSender::UploadLogin(string localSecret, uint8_t* fileNameHash)
{
    ...
    msgBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + CHUNK_HASH_SIZE + CHUNK_HASH_SIZE);
