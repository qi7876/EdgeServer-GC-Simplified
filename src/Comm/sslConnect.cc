...
SSLConnection::~SSLConnection() {
    tool::Logging(myName_.c_str(), "destroy the SSLConnection object.\n");
...
bool SSLConnection::SendData(SSL* connection, uint8_t* data, uint32_t dataSize) {
    int writeStatus;
    //tool::Logging(myName_.c_str(), "write data size first\n");
    writeStatus = SSL_write(connection, (char*)&dataSize, sizeof(uint32_t));
    //tool::Logging(myName_.c_str(), "write the dataSize: %d\n", writeStatus);
    if (writeStatus <= 0) {
    ...
    int sendedSize = 0;
    //tool::Logging(myName_.c_str(), "write data then \n");
