...
void ServerOptThread::Run(SSL *clientSSL, int migrateFileNum)
{
    ...
    if (optType == UPLOAD_OPT)
    {
        ...
        if (uploadFileList.size() == migrateFileNum)
        {
            ...
            tool::Logging(myName_.c_str(), "migrator start\n");
            sleep(1);
            enclaveMigrationObj_->MigrateFileToCloud(uploadFileList);
            sleep(1);
            tool::Logging(myName_.c_str(), "migrator end\n");

            // tool::Logging(myName_.c_str(),"gc start\n");
            // sleep(1);
            // Ocall_ClearFpIndex();
            // enclaveMigrationObj_->MigrateDeleteChunk();
            // sleep(1);
            // tool::Logging(myName_.c_str(),"gc end\n");
        }
    }
 ...
 *
 */
void ServerOptThread::GarbageCollection()
{
    tool::Logging(myName_.c_str(), "garbage collection start\n");
    sleep(1);
    Ocall_ClearFpIndex();
    enclaveMigrationObj_->MigrateDeleteChunk();
    sleep(1);
    Ocall_SaveFpIndex();
    tool::Logging(myName_.c_str(), "garbage collection end\n");
    return;
}