...
void EnclaveMigrator::DownloadChunkFromCloud(string fileName)
{
    ...
    thTmp = new boost::thread(attrs, boost::bind(&DataWriter::Run, dataWriterObj_, outClient_RT->_inputMQ_RT));
    // init enclave client
...
void EnclaveMigrator::MigrateDeleteChunk()
{
    ...
    boost::thread_attributes attrs;
    attrs.set_stack_size(THREAD_STACK_SIZE);
    thTmp = new boost::thread(attrs, boost::bind(&DataWriter::Run, dataWriterObj_, outClient_GC->_inputMQ_GC));
