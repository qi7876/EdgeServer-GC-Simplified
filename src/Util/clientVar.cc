...
void ClientVar::InitUploadBuffer()
{
    ...
    _outQuery.outQueryBase = (OutQueryEntry_t*)malloc(sizeof(OutQueryEntry_t) * sendChunkBatchSize_);
    _outQuery.queryNum = 0;

    // init the recv buffer
    _recvChunkBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendChunkBatchSize_ * sizeof(Chunk_t));
    ...
    _outRecipe.entryList = (uint8_t*)malloc(sendRecipeBatchSize_ * CHUNK_HASH_SIZE);
    _outRecipe.recipeNum = 0;

    _outSecureRecipe.entryList = (uint8_t*)malloc(sendRecipeBatchSize_ * CHUNK_HASH_SIZE);
    _outSecureRecipe.recipeNum = 0;

    _outKeyRecipe.entryList = (uint8_t*)malloc(sendRecipeBatchSize_ * MLE_KEY_SIZE);
...
void ClientVar::InitMigrateBuffer()
{
    ...
    _sendSecRecipeBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendRecipeBatchSize_ * sizeof(RecipeEntry_t));
    ...
    _sendKeyRecipeBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendRecipeBatchSize_ * sizeof(keyRecipeEntry_t));
    ...
    // init chunk buffer
    _sendChunkBuf_MR.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendChunkBatchSize_ * sizeof(Chunk_t));
    ...
    // container buffer
    _reqContainer_MR.idBuffer = (uint8_t*)malloc(CONTAINER_CAPPING_VALUE * CONTAINER_ID_LENGTH);
    _reqContainer_MR.containerArray = (uint8_t**)malloc(CONTAINER_CAPPING_VALUE * sizeof(uint8_t*));
    _reqContainer_MR.idNum = 0;
    for (size_t i = 0; i < CONTAINER_CAPPING_VALUE; i++) {
        _reqContainer_MR.containerArray[i] = (uint8_t*)malloc(sizeof(uint8_t) * MAX_CONTAINER_SIZE);
    }
...
void ClientVar::InitRetrieveBuffer()
{
    ...
    _recvRecipeBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendRecipeBatchSize_ * sizeof(RecipeEntry_t));
    ...
    _recvChunkBuf_RT.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + recvChunkSize * sizeof(Chunk_t));
    ...
    _sendSecFpBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendRecipeBatchSize_ * sizeof(RecipeEntry_t));
...
void ClientVar::InitRestoreBuffer()
{
    ...
    _sendChunkBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sendChunkBatchSize_ * sizeof(Chunk_t));
...
void ClientVar::InitGCBuffer()
{
    ...
    // container buffer
    _reqContainer_GC.idBuffer = (uint8_t*)malloc(CONTAINER_CAPPING_VALUE * CONTAINER_ID_LENGTH);
    _reqContainer_GC.containerArray = (uint8_t**)malloc(CONTAINER_CAPPING_VALUE * sizeof(uint8_t*));
    _reqContainer_GC.idNum = 0;
    for (size_t i = 0; i < CONTAINER_CAPPING_VALUE; i++) {
        _reqContainer_GC.containerArray[i] = (uint8_t*)malloc(sizeof(uint8_t) * MAX_CONTAINER_SIZE);
    }
