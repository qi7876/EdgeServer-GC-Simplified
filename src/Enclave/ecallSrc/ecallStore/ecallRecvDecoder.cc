    ...
    while (processNum < recipeNum) {
        ...
        if (reqContainer->idNum == CONTAINER_CAPPING_VALUE || processNum == recipeNum) {
            ...
            while (startEntry != endEntry) {
                ...
                // chunk buf已满，准备传送
                if (restoreChunkBuf->header->currentItemNum % Enclave::sendChunkBatchSize_ == 0 || processNum == recipeNum) {
    ...
    unordered_map<string, RestoreIndexEntry_t>& restoreIndex, uint8_t** containerArray, uint32_t idNum)
{
    ...
    for (size_t i = 0; i < idNum; i++) {
        ...
        for (size_t j = 0; j < chunkNum; j++) {
            ...
            tmpRestoreEntry.offset = (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 3]) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 2] << 8) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 1] << 16) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 0] << 24) + chunkNum * (CHUNK_HASH_SIZE + 8) + 4;
            // Enclave::Logging(myName_.c_str(), "ok for get offset\n");
            tmpRestoreEntry.length = (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 7]) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 6] << 8) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 5] << 16) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 4] << 24);
