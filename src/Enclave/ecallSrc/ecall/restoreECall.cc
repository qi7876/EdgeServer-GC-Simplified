 ...
 */
void Ecall_ProcRecipeBatch(uint8_t* recipeBuffer, uint8_t* keyRecipeBuffer, size_t recipeNum,
    ResOutSGX_t* resOutSGX) {
    ecallRecvDecoderObj_->ProcRecipeBatch(recipeBuffer, keyRecipeBuffer, recipeNum, 
        resOutSGX);
    return ;
}
