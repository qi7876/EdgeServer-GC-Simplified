    ...
    uint8_t* encMasterKey, void** sgxClient)
{
...

    {
        ...
        if (optType != MIGRATE_TO_CLOUD && optType != DOWNLOAD_FROM_CLOUD && optType != GC_OPT) {
#if (MULTI_CLIENT == 1)
...
#if (SGX_BREAKDOWN == 1)
    double rawOcallTime = enclaveBaseObj_->_testOCallTime / static_cast<double>(enclaveBaseObj_->_testOCallCount);
    double MS_TO_USEC = 1000.0;
    info->dataTranTime = (enclaveBaseObj_->_dataTransTime - (enclaveBaseObj_->_dataTransCount * rawOcallTime)) / MS_TO_USEC;
    info->fpTime = (enclaveBaseObj_->_fingerprintTime - (enclaveBaseObj_->_fingerprintCount * rawOcallTime)) / MS_TO_USEC;
    info->freqTime = (enclaveBaseObj_->_freqTime - (enclaveBaseObj_->_freqCount * rawOcallTime)) / MS_TO_USEC;
    info->firstDedupTime = (enclaveBaseObj_->_firstDedupTime - (enclaveBaseObj_->_firstDedupCount * rawOcallTime)) / MS_TO_USEC;
    info->secondDedupTime = (enclaveBaseObj_->_secondDedupTime - (enclaveBaseObj_->_secondDedupCount * rawOcallTime)) / MS_TO_USEC;
    info->compTime = (enclaveBaseObj_->_compressTime - (enclaveBaseObj_->_compressCount * rawOcallTime)) / MS_TO_USEC;
    info->encTime = (enclaveBaseObj_->_encryptTime - (enclaveBaseObj_->_encryptCount * rawOcallTime)) / MS_TO_USEC;
