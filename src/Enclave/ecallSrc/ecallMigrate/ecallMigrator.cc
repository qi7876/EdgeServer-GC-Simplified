    ...
    InMigrationEntry_t* startEntry, size_t entryNum, SendMsgBuffer_t* sendChunkBuffer)
{
    ...
    for (size_t i = 0; i < idNum; i++) {
        ...
        size_t chunkNum = (containerArray[i][3]) + (containerArray[i][2] << 8) + (containerArray[i][1] << 16) + (containerArray[i][0] << 24);
        ...
        for (size_t j = 0; j < chunkNum; j++) {
            ...
            if (result != tmpMap[i].end()) {
                uint32_t offset = ((headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 3]) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 2] << 8) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 1] << 16) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 0] << 24) + chunkNum * (CHUNK_HASH_SIZE + 8) + 4);
                uint32_t length = ((headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 7]) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 6] << 8) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 5] << 16) + (headerBuf[j * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 4] << 24));
            ...
            if (sendChunkBuffer->header->currentItemNum != 0 && sendChunkBuffer->header->currentItemNum % Enclave::sendChunkBatchSize_ == 0) {
        ...
        if (sendChunkBuffer->header->currentItemNum % Enclave::sendChunkBatchSize_ != 0 && i == idNum - 1) {
            sendChunkBuffer->header->messageType = EDGE_MIGRATION_CHUNK;
            Ocall_SendMigrationData(mrOutSGX->outClient);
            ...
            if (outGC->queryNum == Enclave::sendRecipeBatchSize_ || j == chunkNum - 1) {
                ...
                for (size_t l = 0; l < outGC->queryNum; l++) {
                    ...
                    if (outGCEntry->existFlag == EXIST) {
                        uint32_t offset = (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 3]) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 2] << 8) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 1] << 16) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 0] << 24) + chunkNum * (CHUNK_HASH_SIZE + 8) + 4;
                        uint32_t length = (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 7]) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 6] << 8) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 5] << 16) + (headerBuf[k * (CHUNK_HASH_SIZE + 8) + CHUNK_HASH_SIZE + 4] << 24);
